defmodule Publisher.AdminsTest do
  ...
  describe "admins" do
    ...
    test "create_admin/1 with valid data creates a admin" do
      valid_attrs = %{dni: "some dni", name: "some name"}

      assert {:ok, %Admin{} = admin} = Admins.create_admin(valid_attrs)
      assert admin.dni == "some dni"
    ...
    test "update_admin/2 with valid data updates the admin" do
      admin = admin_fixture()
      update_attrs = %{dni: "some updated dni", name: "some updated name"}

      assert {:ok, %Admin{} = admin} = Admins.update_admin(admin, update_attrs)
      assert admin.dni == "some updated dni"
